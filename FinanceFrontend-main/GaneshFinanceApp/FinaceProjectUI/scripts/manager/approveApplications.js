...
async function rejectApplication(appId, custID) {
    try {
        ...
        const response = await fetch('http://localhost:8080/FinanceBackend/rest/manager/onReject', requestOptions);
        if (!response.ok) {
...
function addContent() {
    ...
    if (apps.length == 0) {
        ...
        apps.forEach((app) => {
            ...
            btnApprove.classList.add("btn")
            btnApprove.classList.add("btn-success")
            btnApprove.type = "button"
            btnApprove.value = app.application_number
            ...
            c8.appendChild(btn)
            c10.appendChild(btnApprove)
            c11.appendChild(btnReject)
            row.appendChild(c9);
            row.appendChild(c1);
            row.appendChild(c2);
            row.appendChild(c3);
...
function getApplications() {
    ...
    req.open('GET', 'http://localhost:8080/FinanceBackend/rest/manager/waitingForApproval')
